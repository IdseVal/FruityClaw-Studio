# The ORCA ADE agent workflow: complete setup (v0.1)
...
*Supersedes the v2 setup document. New in v0.1: the loop is CLOSED -- when the pipeline
drains, the dispatcher spawns the PO & Analyst to audit the backlog against the core
document; it either files the missing issues or declares completion by marking the
document `Status: ACHIEVED` through a normal PR, which stops dispatching and pages the
owner. `dispatch.py onboard` becomes revision-aware: with a populated core document it
runs a revision interview that reopens the gate. Also new: dispatcher subprocesses run
with `CREATE_NO_WINDOW`, so the scheduled task no longer flashes console windows.*
...
| core document: `Status: EMPTY` | gate closed; nothing dispatches until the PO interview PR merges |
| core document: `Status: ACHIEVED` | project complete: dispatching stops, human paged; a revision interview reopens |
...
**The human's touchpoints, in total:** the PO interview (first and revision rounds); FMEA
interviews; `needs-human` pings; reviewing the result when the loop declares completion;
opening the `dev -> main` pull request. Everything else runs itself -- including deciding
what to work on next: when the pipeline drains, the dispatcher spawns the PO & Analyst to
audit the backlog against the core document and either file the missing issues or declare
the document achieved. The loop runs until that declaration, and a revision interview
(`dispatch.py onboard`) starts the next round.
...
1. Gate: nothing is dispatched while `docs/CORE_DOCUMENT.md` on `dev` still says
   `Status: EMPTY`. The PO's onboarding PR opens the gate when it merges. The gate also
   closes when the document says `Status: ACHIEVED` (project complete); a revision
   interview reopens it (see Backlog and completion).
...
1. Drained -- no open issues, no open PRs into `dev`, no active worktrees, gate open ->
   spawn the PO & Analyst in a `backlog-audit-<n>` worktree. The audit compares what is
   merged on `dev` against the core document and ends in one of two outcomes, both read
   from GitHub rather than from the agent: new issues (the loop re-enters by itself), or
   a PR flipping the core document status line to `Status: ACHIEVED -- <date>` (reviewed
   and merged like any other docs-only PR).
2. The audit session is watched like any other: idle -> one nudge -> page the human. An
   audit that ends with neither outcome while nothing has merged since (same merged-PR
   high-water mark) is never repeated -- the human is paged instead.
3. `Status: ACHIEVED` on `dev`: dispatching stops and the human is paged once -- review
   the result, open the dev -> main PR. For a new round, `dispatch.py onboard` with a
   populated core document becomes a REVISION interview (new features, changes); its PR
   sets the status back to `Status: AGREED`, which reopens the gate by itself.
...
(first and revision rounds), FMEA interviews, the completion page, and opening the
...
dispatcher:
  ...
  max_active_issues: 10              # issues with a live worktree at once; protects quota and Orca
...
phase -- and it keeps going until the backlog audit of step H declares completion.
*Verify:* `dispatch.py status` shows issues moving without anyone touching GitHub.

**G -- promotion.** The human inspects `dev` and opens the pull request into `main`
themselves. No agent ever does.

**H -- completion and the next round.** When every issue is closed and every PR merged,
the dispatcher notices the drained pipeline and spawns a PO & Analyst backlog audit. It
either files the issues still missing (the loop continues by itself) or -- when the core
document is fully achieved and holds no OPEN items -- opens a PR marking it
`Status: ACHIEVED -- <date>`. Once that merges, dispatching stops and the human is paged.
Restart at any time with `python .orca/dispatcher/dispatch.py onboard`: with a populated
core document it starts a REVISION interview (new features, changes, feedback), and
merging its PR (status back to AGREED) reopens the gate by itself.
*Verify:* after the ACHIEVED merge, the gate line of `dispatch.py status` says `project
complete`; after a revision round it flips back to `OPEN`.
...
[ ] on a drained pipeline: backlog audit spawns; new issues appear or an ACHIEVED PR pages the owner
...
merging into `dev`, closing issues, advancing phases -- and, when everything is done,
auditing its own backlog: the PO & Analyst compares the result against the core document
and either files what is missing or declares the project complete, at which point you are
paged and the machine goes quiet. Your machine being on and Orca being open is all it
needs. Reboots are fine -- the task restarts at logon.

**Your five touchpoints:**

1. **The PO interview** (once per project, plus a revision interview whenever you want a
   new round). An Orca tab opens and interviews you in rounds. Answer, correct, and
   finally say it is agreed.
...
5. **Completion and the next round.** When the loop declares the core document achieved
   you get one page and everything stops. Review the result; promote to `main` when
   satisfied. Want more -- new features, changes, feedback from using it? Run
   `python .orca\dispatcher\dispatch.py onboard`: an Orca tab opens and the PO & Analyst
   interviews you about the new round. When you say it is agreed, the whole loop restarts
   by itself -- nothing else to reset.
