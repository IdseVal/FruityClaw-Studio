...
| O-19.1 | #19 provenance | **Resolved, 2026-08-26** — see [`ADR-062`](../adrs/ADR-062-provenance-two-flags.md) and issue #62. Two distinct flags: **`ai_origin`** (§6.3, the visual mark) on any entity created or materially rewritten by an `origin = Assistant` Delta; **`generative_output`** (§6.2, the licence caveat) only on entities from `generate_pattern` or `generate_sample`. An Assistant-built drum lane is AI-authored and must be marked; it is not generative-model output and must not carry the licence warning. Both are derived from one three-state `authorship` field, so the nesting is unrepresentable-wrong rather than a rule (data model §4, P7). **"Both set from the `Delta`" is discharged by the Function, not by `Delta.origin`**: the generating Function writes the flag as a `Set` inside the same Delta as the content (history contract §7.4), so the Delta never has to name which Function ran (§7.3, NON-scope 6) |
