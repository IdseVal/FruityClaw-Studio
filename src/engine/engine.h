...
//   - Effect Processors are created and prepared on the edit thread by the
//     ProcessorPool, referenced from the model, driven (parameters applied,
//     process() called) only on the audio thread, and retired by epoch like
//     a model when their Effect leaves the Project.
#pragma once
...
public:
    ...
    // Edit thread, before the first publish. Without a factory the Effect
    // chains are silent pass-throughs and the Studio still plays.
    void set_processor_factory(ProcessorFactory factory);
    ...
    // The largest block a Processor is prepared for; longer callbacks are
    // rendered in slices of this.
...
private:
    ...
    void render_block(float* const* output, int output_channels, int frames);
    ...
    void render_instrument(const RenderModel& model, std::uint32_t instrument, int frames);
    void apply_chain(const std::vector<BakedEffect>& chain, float* const* bus, int frames);
    static void take_up_parameters(const RenderModel& model);
    ...
    // One Instrument's voices summed, then its chain; then summed into the
    // mix, which the master chain processes. Stereo throughout; a mono
    // output takes the left channel.
    std::array<float, kMaxBlockFrames> bus_l_{}, bus_r_{}, mix_l_{}, mix_r_{};

    // --- edit-thread state --------------------------------------------------
    struct Retired {
        std::shared_ptr<const void> object;  // a RenderModel, an AuditionCue or a Processor
        std::uint64_t epoch;
    };
