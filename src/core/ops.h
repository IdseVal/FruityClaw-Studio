...
//
// An Op mutates the MusicalContent and returns its exact inverse, built from
// the state actually observed at the moment of application — never from a
...
//
// Ops are typed on MusicalContent, not on Project (ADR-060). A Delta is
// therefore, by its type, a change to the musical content and to nothing
// else: no Delta can reach the Project's identity, its file path or its save
// metadata, whoever built it.
#pragma once
...
struct Op {
    // Mutates the musical content and returns the inverse Op, or nullopt
    // when application observed no change (a Set whose value was already
    // current).
    std::function<std::optional<Op>(MusicalContent&)> run;
...
// back in reverse and the OpError is rethrown — the content is left unchanged.
...
// `get(content)` returns the current value; `set(content, value)` writes it.
template <typename T, typename GetFn, typename SetFn>
Op make_set(GetFn get, SetFn set, T after, std::size_t bytes = sizeof(T)) {
    Op op;
    op.bytes = bytes;
    op.run = [get, set, after = std::move(after), bytes](MusicalContent& p) -> std::optional<Op> {
        T before = get(p);
