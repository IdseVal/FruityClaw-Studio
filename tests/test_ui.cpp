...
// from the built file, and the step sequencer (issue #9): drawing,
// painting, erasing and velocity as single undo entries, lane mute, new
// Pattern, rename, and the provenance mark.
//
...
TEST_CASE("only an AI-generated Pattern carries the mark when opened in the sequencer") {
    SequencerFixture f;
    ...
    // The same label the palette row carries (see "only an AI-generated
    // Pattern carries the provenance mark in the palette" above), so the
    // two surfaces are marked identically.
    CHECK(badge->accessibleName() == ui::kProvenanceMarkLabel);
    CHECK(name->accessibleDescription() == ui::kProvenanceMarkLabel);
