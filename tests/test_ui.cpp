...
// and the split between the section 6.3 mark and the section 6.2 licence
// caveat (issue #62, ADR-062),
...
struct BrowserFixture {
    ...
    static Project with_generated(Project project, Id& id) {
        Sample generated{new_id(), "Riser", test_support::make_tone(0.2, 440.0),
                         Provenance::generative_model("test-model", 1700000000)};
        id = generated.id;
...
Project with_generated_melody(Project project, Id melody) {
    project.patterns.find(melody)->provenance = Provenance::generative_model("test-model", 1700000000);
    return project;
}
...
TEST_CASE("the shared provenance line is worded once, for Samples and Patterns alike") {
    ...
    CHECK(ui::provenance_text(Provenance::generative_model("test-model", 1700000000)) ==
...
TEST_CASE("the licence caveat separates from the mark: O-19.1's two flags") {
    // ADR-062. Three states, and the section 6.2 sentence appears on exactly
    // one of them. An Assistant edit of the user's own material is section 6.3
    // AI origin, not generative output; telling its author it "may not be
    // licenseable" would retract the section 6.1 promise.
    Provenance hand = Provenance::human();
    Provenance assisted = Provenance::assistant("test-assistant", 1700000000);
    Provenance generated = Provenance::generative_model("test-model", 1700000000);

    CHECK_FALSE(hand.ai_origin());
    CHECK_FALSE(hand.generative_output());
    CHECK(assisted.ai_origin());
    CHECK_FALSE(assisted.generative_output());
    CHECK(generated.ai_origin());
    CHECK(generated.generative_output());

    CHECK(ui::provenance_text(assisted) == "Made with the Assistant (test-assistant) on 2023-11-14.");
    CHECK_FALSE(ui::provenance_text(assisted).contains("licenseable"));
    CHECK(ui::provenance_text(generated).contains("licenseable"));
}

TEST_CASE("an Assistant-edited Pattern is marked in the palette without the licence sentence") {
    test_support::Fixture ids = make_fixture();
    ids.project.patterns.find(ids.melody_pattern)->provenance =
        Provenance::assistant("test-assistant", 1700000000);
    ProjectHistory history(std::move(ids.project));
    ui::PatternPalette palette(history);
    palette.show();
    REQUIRE(palette.count() == 2);

    // The section 6.3 mark is owed here exactly as it is on generated content.
    CHECK(palette.item(1)->data(Qt::AccessibleDescriptionRole).toString() == "AI-generated");
    CHECK(accent_pixels(palette.item(1)->icon().pixmap(26, 14).toImage()) > 20);
    // The section 6.2 caveat is not.
    CHECK(palette.item(1)->toolTip().contains("test-assistant"));
    CHECK_FALSE(palette.item(1)->toolTip().contains("licenseable"));
    // And the hand-made Pattern still gets neither.
    CHECK(palette.item(0)->toolTip() == "Made by hand");
    CHECK(accent_pixels(palette.item(0)->icon().pixmap(26, 14).toImage()) == 0);
}

TEST_CASE("an Assistant-edited Sample opens with the mark and no licence sentence") {
    auto ids = make_fixture();
    ids.project.samples.items.push_back(Sample{new_id(), "tidied", test_support::make_tone(0.2, 330.0),
                                               Provenance::assistant("test-assistant", 1700000000)});
    ProjectHistory history(std::move(ids.project));
    StubAudition audition;
    ...
    CHECK(list->item(2)->data(Qt::AccessibleDescriptionRole).toString() == "AI-generated");
    CHECK(accent_pixels(list->item(2)->icon().pixmap(36, 22).toImage()) > 20);
    CHECK_FALSE(list->item(2)->toolTip().contains("licenseable"));

    emit list->itemActivated(list->item(2));
    QLabel* provenance = browser.findChild<QLabel*>("provenance");
    REQUIRE(provenance);
    CHECK(provenance->text() == "Made with the Assistant (test-assistant) on 2023-11-14.");
}
