...
TEST_CASE("add_sample appends one Human Sample over the audio and undo removes it") {
    ...
    auto added = add_sample(history.read().musical, "Take 1", take);
    REQUIRE(added.ok());
    REQUIRE(added->delta.ops.size() == 1);
    REQUIRE(history.apply(std::move(added->delta)) == ApplyResult::Applied);

    const auto& samples = history.read().musical.samples.items;
    ...
    CHECK(history.read().musical.instruments == original.musical.instruments);
    CHECK(history.read().musical.patterns == original.musical.patterns);
    CHECK(history.state().undo_label == "Add Sample 'Take 1'");

    REQUIRE(history.undo());
    CHECK(history.read() == original);
    REQUIRE(history.redo());
    CHECK(history.read().musical.samples.items.size() == 3);
}

TEST_CASE("add_sample refuses empty audio") {
    auto f = make_fixture();
    CHECK(add_sample(f.project.musical, "Empty", nullptr).error == "Nothing was recorded");
    CHECK(add_sample(f.project.musical, "Empty", std::make_shared<AudioData>()).error ==
