...
const Track& track(const MusicalContent& p, Id arrangement, Id id) {
...
TEST_CASE("delete_track takes its Placements with it and undo restores both") {
    ...
    MusicalContent with_placement = history.read().musical;
    ...
    CHECK(history.read().musical.patterns.find(f.drum_pattern) != nullptr);

    REQUIRE(history.undo());
    CHECK(history.read().musical == with_placement);
}

TEST_CASE("deleting a middle Track restores it at its position") {
    auto f = make_fixture();
    ProjectHistory history(std::move(f.project));
    auto created = create_track(history.read().musical, f.arrangement, "Third", std::nullopt);
    REQUIRE(created.ok());
    REQUIRE(history.apply(std::move(created->delta)) == ApplyResult::Applied);
    MusicalContent before = history.read().musical;

    auto del = delete_track(history.read().musical, f.arrangement, f.track_b);  // middle
    REQUIRE(del.ok());
    REQUIRE(history.apply(std::move(*del)) == ApplyResult::Applied);
    REQUIRE(history.undo());
    CHECK(history.read().musical == before);  // order preserved bit for bit
}
...
TEST_CASE("add_placement defaults its length to the Pattern's length") {
    ...
    auto placed = add_placement(history.read().musical, f.arrangement, f.track_a, f.melody_pattern,
                                8 * kPpq);
    REQUIRE(placed.ok());
    REQUIRE(history.apply(std::move(placed->delta)) == ApplyResult::Applied);

    const Track& t = track(history.read().musical, f.arrangement, f.track_a);
...
TEST_CASE("removing a middle Placement restores it at its position on undo") {
    ...
    auto removed = remove_placement(history.read().musical, f.arrangement, f.track_a, middle);
    REQUIRE(removed.ok());
    REQUIRE(history.apply(std::move(*removed)) == ApplyResult::Applied);
    CHECK(track(history.read().musical, f.arrangement, f.track_a).placements.size() == 2);
...
TEST_CASE("move_placement within a Track changes start and nothing else") {
    ...
    auto moved = move_placement(history.read().musical, f.arrangement, f.track_a, id,
                                4 * kPpq, f.track_a);
    REQUIRE(moved.ok());
...
TEST_CASE("move_placement across Tracks is one atomic Delta") {
    ...
    CHECK(track(history.read().musical, f.arrangement, f.track_a).placements.empty());
    const Track& to = track(history.read().musical, f.arrangement, f.track_b);
...
TEST_CASE("resize_placement changes length only; the Pattern is untouched") {
    ...
    auto resized = resize_placement(history.read().musical, f.arrangement, f.track_a, id, 8 * kPpq);
    REQUIRE(resized.ok());
    REQUIRE(history.apply(std::move(*resized)) == ApplyResult::Applied);

    CHECK(track(history.read().musical, f.arrangement, f.track_a).placements[0].length == 8 * kPpq);
    CHECK(history.read().musical.patterns.find(f.drum_pattern)->length == 4 * kPpq);
}
...
TEST_CASE("validation failures return an error and touch nothing") {
    ...
    CHECK_FALSE(add_placement(f.project.musical, f.arrangement, new_id(), f.drum_pattern, 0).ok());
    CHECK_FALSE(add_placement(f.project.musical, f.arrangement, f.track_a, new_id(), 0).ok());
    CHECK_FALSE(add_placement(f.project.musical, f.arrangement, f.track_a, f.drum_pattern, -1).ok());
    CHECK_FALSE(rename_track(f.project.musical, f.arrangement, new_id(), "X").ok());
    CHECK_FALSE(resize_placement(f.project.musical, f.arrangement, f.track_a, new_id(), kPpq).ok());
    CHECK(f.project.musical == original);
}
...
TEST_CASE("rename_track changes the name only and undo restores it") {
    ...
    CHECK(track(history.read().musical, f.arrangement, f.track_b).name == "Track 2");
...
TEST_CASE("a no-net-change Delta reports NoChange and keeps the redo stack") {
    ...
    auto same = rename_track(history.read().musical, f.arrangement, f.track_a, "Track 1");
    REQUIRE(same.ok());
    CHECK(history.apply(std::move(*same)) == ApplyResult::NoChange);
    CHECK(history.state().can_redo);
    REQUIRE(history.redo());
    CHECK(track(history.read().musical, f.arrangement, f.track_a).placements.size() == 1);
}
...
TEST_CASE("set_track_muted toggles one flag and a same-value set is a no-op") {
    ...
    CHECK(track(history.read().musical, f.arrangement, f.track_a).muted);
    CHECK_FALSE(track(history.read().musical, f.arrangement, f.track_b).muted);

    auto again = set_track_muted(history.read().musical, f.arrangement, f.track_a, true);
    REQUIRE(again.ok());
...
TEST_CASE("an Assistant-originated Delta carries its Origin") {
    auto f = make_fixture();
    auto created = create_track(f.project.musical, f.arrangement, "Bot", std::nullopt,
                                Origin::Assistant);
    REQUIRE(created.ok());
    CHECK(created->delta.origin == Origin::Assistant);

    auto placed = add_placement(f.project.musical, f.arrangement, f.track_a, f.drum_pattern, 0, 0,
                                Origin::Assistant);
    REQUIRE(placed.ok());
...
TEST_CASE("both Pattern kinds place through the same function without special cases") {
    ...
    for (Id pattern : {f.drum_pattern, f.melody_pattern}) {
        auto placed = add_placement(history.read().musical, f.arrangement, f.track_a, pattern,
                                    0);
        REQUIRE(placed.ok());
        REQUIRE(history.apply(std::move(placed->delta)) == ApplyResult::Applied);
    }
